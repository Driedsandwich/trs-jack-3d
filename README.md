...
| コマンド | 内容 |
|---|---|
| `npm run test` | 単体テスト (Vitest) 83 件 |
...
| 11.78 mm | **橋絡発生**。帰線用接点が第2絶縁帯をまたぎ、**Ring と Sleeve を同時に短絡**する（真値 11.760。上表は 0.02 mm 刻みの走査値） |
...
仮定を成立範囲いっぱいに振って測った結果が次です（[docs/SENSITIVITY.md](docs/SENSITIVITY.md)）。

| 事象 | 上表の値 | 仮定を振ったときの幅 |
|---|---:|---|
| Ring↔Sleeve 橋絡が始まる | 11.76 mm | **6.96〜13.14 mm** |
| Ring 側ブレーク接点が開く | 8.05 mm | **8.00〜12.33 mm**（しきい値 0.4 以上では開かない） |

**一方、「何が起きるか」は幅を持ちません。** 帰線接点が Tip → Ring の順に通過すること、
その過程で Ring↔Sleeve の橋絡が起きること、
**Tip↔Ring の橋絡は起きないこと**、ブレーク接点が正しい結線より先に開くこと ――
これらは仮定をどう選び直しても変わりませんでした。
...
なおこの結論は、**仮定の選び方に依存しません**。接点が Tip 面に届く条件は
`compliance ≥ 0.150 mm`（＝ Ring と Tip の半径差）で、**パッド幅にも接点位置にも
一切依存しない**からです。採用値 0.05 はしきい値 0.1538 の約 1/3.1 です。
しきい値未満の内側を 864 通り埋めても、Tip 橋絡は 1 度も出ませんでした。
**ただしこれは公称寸法での話です。** プラグ外径を図面公差 φ3.5±0.05 の下半分に振ると
しきい値は 0.15 を割ります（[docs/SENSITIVITY.md](docs/SENSITIVITY.md) §6-1）。
