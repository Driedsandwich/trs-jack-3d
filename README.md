...
理由は **ジャックの内部が公開されていない**ことです。
外形はメーカー公開図面どおりですが、**接点の位置・ばね・押付力は図面にもデータシートにも
書かれていません**。そこは仮定で埋めてあり、**実物と突き合わせた検証もまだ済んでいません**
（[UNKNOWNS.md](UNKNOWNS.md)）。

**ただし、何もかもが不確かなわけではありません。**
このアプリの中で、確かなものとそうでないものは、はっきり分かれています。

| | |
|---|---|
| **確か** — 仮定を振り直しても変わらない | 接点が何に触れるかの**順序**（帰線接点が Tip → Ring の順に通る）／その過程で Ring↔Sleeve の橋絡が起きること |
| **不確か** — 仮定に直接乗る | それが**何 mm で**起きるかという深さ／挿抜力の絶対値／ブレーク接点がいつ開くか（しきい値が仮定で、値によっては一度も開きません） |

**✅ の用途がどれも「順序」と「仕組み」の話で、❌ の用途がどれも「値」の話なのは、そのためです。**
仮定の内訳が知りたい場合は [ASSUMPTIONS.md](ASSUMPTIONS.md)（寸法 135 件のうち 54 件が仮定）へ。
...
| `npm run check:published-assets` | **公開済み release asset が公開時のまま在るか**を GitHub 側と突き合わせる（`gh` が要る。→ [SECURITY.md](SECURITY.md#対象の範囲)） |
...
これらは帰線接点の仮定をどう選び直しても変わりませんでした。

> **ブレーク接点の順序はこの「変わらない」に含めません。**下の（※）のとおり、
> 開くしきい値を振ると**値によっては一度も開かなくなる**ためです。
> 2026-08-03 まで、ここに「ブレーク接点が正しい結線より先に開くこと」も並べていました。
...
**配布物は [v0.6.23](https://github.com/Driedsandwich/trs-jack-3d/releases/tag/v0.6.23) です。**
...
### 半挿しで左右差分が残る区間は、モデル上の候補です

`npm run search:topology -- --target DIFFERENCE_SIGNAL` で、接点の軸位置・パッド幅・
接触ドームの追従量を振り、**24,750 構成**を調べました。

> **見出しを 2026-08-03 に直しました。**
> それまで「無改造で左右差分が残ります」と書いていましたが、
> **「無改造」が意味していたのは「このモデルの入力値を 1 つも変えていない」だけ**で、
> 市販の実物がそうなると確かめたわけではありません（統合オーダー P0-5）。

| | 件数 |
|---|---:|
| 完全挿入を壊さずに成立する構成 | **1,338** |
| うちパッド幅の下限 0.3 mm を通ったもの | **879** |
| うち**既定の入力値から 1 つも動かさないもの** | **2** |

| variant | 成立 | 幅 0.3mm 以上 | 既定値のまま | そのときの深さ |
|---|---:|---:|---:|---|
| 3極プラグ × 3極ジャック | 78 | 51 | 0 | — |
| **4極 CTIA × 4極ジャック** | **396** | **252** | **1** | **12.79〜13.00 mm** |
| **3極プラグ × 4極ジャック** | **864** | **576** | **1** | **13.30〜13.52 mm** |
...
4極ジャックの帰線接点が、プラグの絶縁帯にちょうど落ちるためです。

> **「幅 0.3 mm 以上」を「製造しうる」と読まないでください。**
> 0.3 mm は探索空間を切るための下限で、**出典がありません。**
> 材料・ばね応力・耐久性・成形・公差・接触圧・メーカー工程のいずれも確認していません。
> artifact では `passesPadWidthHeuristic` という名前にし、
> `manufacturingVerified: false` を添えています。

#### ⚠ 反対証拠 — 実在資料 2 件が逆を指しています
...
看板の結論は **Tip 接点が 12.6 mm より浅いこと**を必要条件にしています。
PS000001 はその外側です。**どちらが実物に近いかは決着していません**
（[REAL_JACK_COMPARISON.md](docs/REAL_JACK_COMPARISON.md)）。

**4極 CTIA × 4極ジャックはごく普通の組み合わせです**（マイク付きイヤホンを
スマホのジャックへ半挿しにするだけ）。だからこそ、上の不確かさが効きます。

### この結論が乗っている土台

**4極ジャックは Lumberg 1503 28（JEITA RC-5325A 準拠）ベースです**が、
**接点そのものの位置は依然として仮定です。**図面に断面図が無いためです
（[UNKNOWNS.md](UNKNOWNS.md) §5-2）。

| | 区分 |
|---|---|
| 端子 6 本の軸位置 / ブレーク接点 2 個 | **FACT**（図面記載） |
| 端子番号 ↔ 機能の対応 | **DERIVED** |
| **接点そのものの軸位置** | **ASSUMPTION** |

**「実在部品ベースになった」は「実測された」ではありません。**実物を測った人は誰もいません。

**組み直したのは接点系だけです。**外形（ハウジング・ノーズ・ナット）は 3極 1503 09 の値を
流用したままなので、**3D 表示の外形は 1503 28 の実物とは違います**
（1503 28 は SMT のミッドマウント品でナットを持ちません）。

残る仮定は **1 つだけ**です — 接点が端子より何 mm 手前にあるか
（採用値 0、図面と両立する範囲 0〜1.3 mm）。**その全域で区間は出ます。**
動くのは深さだけで、ほぼ 1:1 で移動します。

### ⚠ 実在資料 2 件が逆を指しています

| 根拠 | Tip 接点の軸位置 | 区間 |
|---|---:|---|
| **本モデル**（Lumberg 1503 28 の**端子**位置・メーカー公式） | 11.30 mm | 出る |
| pro-SIGNAL PS000001（**接点**の寸法記入・商社ブランド） | **12.75 mm** | **出ない** |

4極ジャックのデータシートを **13 方面・170 型番**探した結果、
**接点位置そのものが寸法記入された図面は 1 件だけ**でした（PS000001）。
その値を入れると区間は 1 つも出ません。分かれ目は **Tip 接点が 12.6 mm より浅いこと**です。

**証拠の種類が違うので、資料だけでは決まりません。**
本モデルは前者を採っていますが、**それは選択であって決着ではありません。**

経緯・図面の照合方法は
[docs/REAL_JACK_COMPARISON.md](docs/REAL_JACK_COMPARISON.md)（`npm run compare:real-jack`）。
実物で確かめる手順は [docs/VERIFICATION_PLAN.md](docs/VERIFICATION_PLAN.md) §2-2 です。
...
| 区分 | 件数 | 意味 |
|---|---:|---|
| FACT | 53 | メーカーデータシートに数値として直接記載されている |
...
- 挿抜力は**有限要素解析ではありません**。エネルギー法による近似で、摩擦係数とばね定数は仮定値です。
  **とくにばね定数は、完全接点あたりの押付力が「別メーカーの技術資料」の範囲に収まるよう
  逆算した値です。**モデル化した Lumberg 1503 09 自身のばね諸元は公開されていません
  （[SOURCES.md](SOURCES.md) §6。この値を 1503 09 の仕様として表示してはいません）。
  曲線全体は、ジャック側データシートの公称挿抜力レンジに合わせて校正しています。
...
> ジャック内部の寸法は 32 件の仮定を含み（仮定は全体で 54 件）、
