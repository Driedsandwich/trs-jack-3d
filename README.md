...
**ストローク 14 mm に対して ±3 mm 動きます。**下表の 2 桁の数字を確定値として
持ち帰らないでください（測り方は [docs/SENSITIVITY.md](docs/SENSITIVITY.md)）。
...
> 走査は刻みの倍数でしか値を出せないので、真の切り替わりはその刻みの間にあります。
> 二分法で詰めた真値が分かっている 2 行には、括弧で併記しました。
...
| 11.78 mm | **橋絡発生**。帰線用接点が第2絶縁帯をまたぎ、**Ring と Sleeve を同時に短絡**する（真値 11.760） |
...
採用している追従量は **0.05 mm** で、届き始める境目 **0.1538 mm** の約 1/3.1。
