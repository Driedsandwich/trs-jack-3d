...
- **手元で確かめられないことは、確かめられないと書きます。**
  ...
  **GNU tar 側でだけ根拠が取れる 12 件 ／ bsdtar 側でだけ取れる 9 件 ／
  どちらでも取れていない 8 件**です（2026-08-11 実測）。
  **「どちらでも取れていない」が v15 で 2 → 8 件へ増えました。**
  増えた 6 件は新しく直した箇所ではなく、**材料を足したときに
  「前から根拠なく止めていたもの」をこの試験自身が炙り出した**ものです
  （見え方を変える鍵の方針・終端 zero block・通常ファイルの重複）。
