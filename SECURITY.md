...
- v0.6.0（`toolVersion` 5）で、信頼できない archive に対して安全に止まるようにしましたが、
  ...
  **その差分試験自体が片方向でした。**「検算が返したものが展開木に在るか」しか見ておらず、
  **「展開されるのに検算が数えない」欠陥は素通り**しました（v9 で逆向きを足しています。
  [verify-tool-v9-notes.md](docs/release/verify-tool-v9-notes.md)）。
