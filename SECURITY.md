...
- **正当な archive を拒む欠陥が、6 版続けて見つかりました。**
  ...
  **塞ぎすぎは「実物が通る」確認では見つかりません**——この repo の実物は
  ...
  拒んでいました（v15 で修正）。
  v15 では corpus の「通す」材料を 59 個へ増やしました（v0.6.6 時点は 9 個）。
...
- **手元で確かめられないことは、確かめられないと書きます。**
  **止める理由の半分は、片方の実装だけでは見えません。**
  その run の 2 実装がそろって通すのに止めているものが 29 件あり、内訳は
  **GNU tar 側でだけ根拠が取れる 15 件 ／ bsdtar 側でだけ取れる 9 件 ／
  どちらでも取れていない 5 件**です（2026-08-11 実測）。
