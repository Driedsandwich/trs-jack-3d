...
const index = {
  ...
  notes: [
     ...
     */
    /**
     * ⚠️ **版数が「契約の版」と読めない書き方にする（v0.6.22・外部監査 P2）。**
     * 「検算ツールの契約（v0.6.16）」は、直後に `toolVersion 20` を書いていても
     * **「これは v0.6.16 の契約だ」と読める。**版数は**いつこの形にしたか**であって、
     * 契約の版ではない。生成値（digest と toolVersion）が正本である。
     */
    `**検算ツールの契約**（この形にしたのは v0.6.16）。**信頼の起点は assets の `
     ...
     */
    /**
     * ⚠️ **件数を手で書かない（v0.6.22・外部監査 P2）。**
     * 「**4 点**」と書いて**ファイルを 5 つ**並べていた。一覧は `RELEASE_ASSETS`
     * から引いているのに**数だけ手書き**だったので、profile が 2 本になった時点でずれた。
     * 数が要るなら一覧から数える。ここでは**数を言わない**のが正しい
     * ——受け手が固定するのは「名指しされた配布物すべて」であって、個数ではない。
     */
    `**受け手が固定すべき配布物**（\`assets[]\` の filename と sha256 を lock すること）: `
