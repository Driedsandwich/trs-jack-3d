...
開発機（macOS）に GNU tar と BusyBox はありません。**bsdtar 3.5.3 と Python 3.14 の 2 実装**で測りました。
監査は GNU tar 1.35 / BusyBox 1.37 / libarchive 3.7.4 / python の 4 実装で測っています。

**指摘は 6 群あり、そのうち開発機で再現できたのは 4 群です。**

**残りは CI で測りました。**この版から CI を GNU tar（ubuntu）と bsdtar（macOS）の
matrix にしたので、**開発機で再現できなかったものにも裏が取れています**（下の 7 と 8）。
BusyBox はまだ入っていません。
...
PAX linkpath → GNU K ／ GNU K → PAX linkpath
  ...
  → CI の ubuntu 側で測ったら、**GNU K → PAX linkpath の順序は割れた**
    （GNU tar 1.35 と python が違う木を作る）。逆順は GNU tar でも割れない
```
...
**uid / gid の上限 2^32−1 は開発機では再現できませんでした**（2 実装とも 2^64 でも通す）。
**CI の ubuntu 側で裏が取れました**——GNU tar 1.35 は
`Extended header gid=4294967296 is out of range 0..4294967295` で exit 2 になり、
**監査の報告とメッセージまで一致しています**（下の 7）。
...
**CI の ubuntu 側でも、この 2 つは GNU tar が通しました。**
勧告に従わなかった判断は、2 実装ではなく 3 実装で裏が取れたことになります。
...
## 7. **止める理由の半分は、片方の実装だけでは見えませんでした**

止めているもののうち、**その run の 2 実装がそろって通し、同じ木を作るもの**を表にしてあります
（`test/tarExtractionOracle.test.ts` の `EVIDENCE_ELSEWHERE`）。
最初は macOS（bsdtar / python）だけで作り、**12 件**でした。

**CI を matrix にした最初の run で、これが片手落ちだと分かりました。**
...
| どこで取れるか | 件 | 中身 |
|---|---:|---|
| **GNU tar のみ** | 9 | PAX の値が数値でない 4 件（`Malformed extended header`）・uid/gid が 32bit 超 2 件（`is out of range 0..4294967295`）・`mtime=+1`・名前と指す先の上書きが 2 つ効く形が各 1 件（片方の順序だけ木が割れる） |
| **bsdtar のみ** | 8 | PAX の `g` 上書き 2 件（bsdtar と python が**別の木**を作る）・`uname`/`gname` の不正 UTF-8 2 件（libarchive が locale 変換で拒む）・パスの不正 UTF-8 4 件 |
| **どちらでも取れていない** | **3** | 名前・指す先の上書きの**片方の順序** 2 件・`..\evil.txt`（Unix ではふつうの名前、Windows では 1 階層上） |

**監査が報告した GNU tar の挙動は、そのとおりでした。**
メッセージまで一致しています（`Extended header gid=4294967296 is out of range 0..4294967295`）。
**こちらが「書式にもとづく判断」と書いていた `mtime=+1` も、GNU tar は拒みました**——
実測で裏が取れたので、判断の根拠が推論から実測へ変わりました。
...
> これが matrix を入れた理由そのもので、入れて初めて測れました。

表は `on`（どちらで根拠が取れるか）を持ち、**毎 run 両方向で照合します。**
自分の実装で取れると書いてあるのに取れなければ落ち、
別の実装で取れると書いてあるのに**こちらでも**取れたら「表が古い」として落ちます。
...
（ubuntu では GNU tar 1.35、macOS では bsdtar 3.5.3 / libarchive 3.7.4）。
実装と版は job のログに残します。

### 最初の run で分かったこと（2026-08-10）

```
macos-latest（bsdtar）  緑   990 件すべて
ubuntu-latest（GNU tar） 赤   8 件失敗 / 982 件成功
```

**赤くなった 8 件は、こちらの過剰拒否ではありませんでした。**
どれも「③ 壊れているの根拠」の試験で、**その試験の書き方が片方の platform しか見ていなかった**ためです
（上の 7）。archive の扱いは変えず、表と試験を両 platform 対応に直しました。

### 2 回目の run で、もう 1 つ見つかりました

表を両 platform 対応に直したら、ubuntu 側の失敗は 8 件 → **1 件**になりました。
残った 1 件は**検算ツールではなく、木を歩く試験の道具の欠陥**でした。

```
Error: ENOENT: no such file or directory, lstat '/tmp/.../pkg-0000000/file<FFFD>.txt'
```

**Linux の file system は、不正な UTF-8 のファイル名を作れます。**
`readdirSync` を既定（utf8）で読むと、そのバイト列が U+FFFD へ置換された文字列で返り、
**その名前で `lstat` すると存在しません。**macOS の APFS はそういう名前を作れないので、
**この欠陥は macOS では出ません。**

置換したまま進めるのはもっと悪くて、**違うバイト列が同じ名前に見える**ようになります。
それは v0.6.4 で検算器の側を直したのと同じ誤りです。
木を歩く側もバイト列で扱い、復元できないバイト列は潰さずそのまま鍵にしました。

> **同じ試験が、OS によって見えるものを変えていました。**
> matrix にしなければ、この 2 つはどちらも見つかりません。

**通す材料（`ok: true` の 24 件）は 1 件も落ちていません。**
GNU tar も、こちらが「止めてはいけない」と決めた形をすべて展開できます——
心配していた 3 件（`uname` の NUL・`comment` の不正 UTF-8・`K` と `L` の併用）も通りました。
