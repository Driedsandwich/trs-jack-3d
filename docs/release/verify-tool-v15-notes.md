...
**手元の 2 実装（macOS の bsdtar / python）は sneaky.ts を作りません。**
そのため**開発機では「実装が割れる」ことを再現できませんでした。**

> **これを「根拠が取れていない」と書いて出したのですが、間違いでした。**
> CI（ubuntu）で回すと、**GNU tar はこの形をちょうど名指しで警告します。**
>
> ```
> tar: A lone zero block at 3
> ```
>
> 材料の並び（0 起点で block 2 が zero block）とも一致します。
> **止める根拠はちゃんとありました**——開発機に無い実装が持っていただけです。
> **2 実装で足りるかどうかは、2 実装では決められない**という同じ話が、また出ました。

この道具の約束は「**ここに挙げた物が中身の全部**」です。
...
## 5. **止める理由のうち、根拠が無いものが増えました（2 → 5 件）**
...
**増えた 3 件は、新しく直した箇所ではありません。**
材料を足したときに、**この試験自身が「前から根拠なく止めていたもの」を炙り出しました。**

```
nonzero-size                        「見え方を変える鍵は解釈しない」という方針
dup-regular-same/different-content   v0.6.1 からの方針（どちらを検算したか言えなくなる）
```

> **最初はここに 8 件と書き、`hdrcharset` と終端 zero block の 3 件も
> 「根拠なし」に入れていました。CI の ubuntu 側が落として、
> どれも GNU tar が警告を出していると分かりました。**
