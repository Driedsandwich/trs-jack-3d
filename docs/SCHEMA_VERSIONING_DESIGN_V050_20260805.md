...
状態: **v0.5.0 で実装済み。**（この文書は設計時の記録である。
設計と実装の差は §6-2 に残してある——本文の数字を後から書き換えていないので、
**§1-5 の「7 件」は設計時の見積もりで、実測は 9 件**である。）
...
1. **条文の機械判定は保守的な近似である。**JSON Schema の言語包含は一般には決定不能なので、
   ...
   > **【2026-08-05 訂正】**この項目にはもともと
   > 「逆方向（上げるべきなのに据え置く）は、対応表に無いキーワードを検出して落とすことで塞いでいる」
   > と書いていた。**これは誤りだった。**外部監査が `oneOf` の反例を出した。
   >
   > ```
   > 旧  oneOf: [{integer}, {number, minimum: 0}]
   > 新  oneOf: [{integer}, {number, minimum: 1}]
   > 値 0    旧 invalid（2 枝一致）→ 新 valid   **広がっている**
   > ```
   >
   > `oneOf` は「ちょうど 1 枝」なので枝の言語について単調でなく、枝を index 同士で
   > 比較していた実装は **HOLD_RECORD**（据え置き可）を返していた。**危険側の誤りである。**
   > v0.5.1 で `oneOf` は変更があれば無条件 UNDEC へ倒した。
...
## 6-2. 実装で分かったこと（2026-08-05 追記・**設計との差**）

設計を実装したときに、設計時の見積もりと**3 点ずれた**。黙って直さず、差として残す。

| | 設計（この文書の本文） | 実装で実測 |
|---|---|---|
| 遡って記録する違反 | **7 件** | **9 件** |
| source snapshot の形と大きさ | tar.gz **0.21 MB** | **JSON 1.05 MB** |
| `role` の enum | 変えない | **`contract-migration` を 1 つ追加** |

### ① 違反は 7 件ではなく 9 件だった

設計では v0.3.0 → v0.4.0 → v0.4.1 の 3 tag しか比べていなかった。
実装時に**全 tag 対（v0.1.0 から v0.4.1 まで）を機械で洗った**ところ、
`half-plug-topology-profile.v1` 系統に 2 件多く見つかった。

```
v0.1.0→v0.1.1  profile v1  spreadStatus の値 3 つを改名 + eventSpreadSource 追加
v0.1.1→v0.2.0  profile v1  inputSettings 追加 + sensitivitySummary を拡張
```

後者は **v2 を新設したのと同じ release で v1 の schema ファイルも直していた**もので、
v0.2.0 が配った artifact は v2 なので実害は無かった。それでも条文には反するので記録した。

**設計時に「3 tag だけ見れば足りる」と決めた根拠は無かった。**
`test/contractMigration.test.ts` の検査③に全 tag 対の走査を入れて、
同じ取りこぼしが次に起きないようにした。

### ② snapshot は tar.gz にできなかった

**`tar.gz` は同じ入力から作っても毎回ハッシュが変わる**（mtime が入る）。
実測で 2 回作って不一致だった。release asset は sha256 で固定するので使えない。

JSON にしたので圧縮が効かず **0.21 MB → 1.05 MB** になった。
それでも tag source archive（8.94 MB）の 1/8 で、
**限界の申告（`isSelfConsistencyOnly` 等）を artifact 自身に持たせられる**利点が加わった。

### ③ 正本ファイルが入力に増えたので `role` を 1 つ足した

`contract-migration.v1.json` が生成器の入力になったため、`roleOfInput` が `other` を返す。
`other` は enum に無いので artifact が schema を通らない。
**`contract-migration` を enum に足し、v0.5.0 の history に記録した**（設計に無い変更）。

### ④ 実装中に見つけた既存の空振り

`test/sourceInputScope.test.ts` の記録漏れ回帰試験は **v0.3.0 tag の source** に当てていた。
台帳が記述しているのは現在の入力なので、**その tag に無いファイルは落としても検出しようがない。**
v0.5.0 で増えた入力を落としても `OK` が返った。台帳も検証対象も現在へ揃え、
**無変異の対照**（何も落とさなければ `OK`）を先に置いた。
