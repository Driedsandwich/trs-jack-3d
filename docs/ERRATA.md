...
## v0.6.22（2026-08-16 記載・**同日直しました**・外部監査）

### 30. **「解決できなかった」を「変わっていない」と同じ扱いにしていました**

| | |
|---|---|
| 対象 | `scripts/schemaLanguageDiff.mjs`（**配布物ではありません**。版を上げるかを決める判定器） |
| 症状 | `$ref` を辿れない・深すぎる・扱えない形のとき、**黙って諦めて「変わっていない」と答える** |
| 影響 | **言語が広がっているのに「据え置き可」と答える。**§25・§26 と同じ危険側 |
| 見つけた人 | **外部監査**（4 件）＋ **こちら**（数え直して 2 件） |
| 直した版 | 2026-08-16（`v0.6.22` 公開後・次の版に入ります） |

**§26 で直した根の隣にありました。**§26 は「節の文字列が変わっていない」を
「意味が変わっていない」の証拠にしていた形。今回は
**「確かめられなかった」を「変わっていない」と同じ扱いにしていた**形です（fail-open）。

**諦めた側と諦めなかった側の出力が同じ**なので、新旧そろって諦めると「同じ」に見えます。

```
逃げ道                                        反例をくれた人
E1 expandRefs の深さ上限を超えた                 こちら（数え直して発見）
E2 循環を見つけた                               こちら（同上）
E3 $ref を解決できない                          監査（55 段の連鎖）
E4 参照先が object でない（boolean schema）      監査
E5 deref が RFC 6901 の ~0 / ~1 を復号しない     監査
...
**直し方。**解決できるものは解決し（E5 の復号・E4 の boolean 展開・E6 の位置ごと比較）、
**解決できないものは「変わっていない」と言わない**（E1・E2・E3）。
諦めが起きたときだけ **root どうしを丸ごと比べ**、まったく同じなら `HOLD`、
違えば決められない側へ倒します——**諦めを一律 BUMP にすると、
深いだけの schema を自分自身と比べても落ちて**条文が使えなくなるためです。

**回帰試験を先に入れてから直しました**（条文 ①-c7 〜 ①-c12。入れた時点で 5 件落ちます）。

**変異対照で 3 か所が「外しても落ちない行」でした。**
E1・E2・E5 は、消しても 93 件すべて通りました。理由も測っています。

- **E1** は監査の反例（55 段）が `deref` の反復上限（E3）を先に踏むため。
  `oneOf` の枝の中で 60 段ネストし、その奥に `$ref` を置くと E1 を踏みます（条文 ①-c11）。
- **E2** は反例をもらっていません。**循環がある schema は保守側へ倒す**という
  振る舞いそのものを固定しました（条文 ①-c12）。
- **E5** は**安全のためでなく精度のため**でした。復号しなくても諦めて root 比較へ落ちるので
  危険側の誤りは出ませんが、escaped `$ref` を使う schema は**参照先が変わっていなくても
  無関係な definition が動くたびに BUMP** になります（条文 ①-c8 ⑤）。

足したあと、**7 か所すべてが変異で落ちます**（99 件）。

**試験の側も直しました。**v0.6.22 の生成器はこの 4 形を一度も作っていません
（実測 300 種で tuple 0 件・`$ref`→boolean 0 件・`~0~1` 0 件・要素が null の配列 0 件）。
補強後は 12 / 9 / 20 / 43 件になり、**v0.6.22 の判定器へ当てると自力で反例を出します**
（seed 299・tuple 形式の 2 番目の枝が `$ref`・証人 `[0,null]`・手書き fixture 抜き）。

**現行 schema への影響はありません。**この 4 形は現行 26 schema に 1 件もありません
（tuple items 0 / `$ref`→boolean 0 / `~0~1` を含む `$ref` 0 / 循環 0 /
